...
@section('title', 'Catat Peminjaman - Inventaris Peminjaman Barang')
@section('page-title', 'Catat Peminjaman')

@section('content')
<div class="card">
    <div class="card-header">
        <h2><i class="fas fa-plus"></i> Catat Peminjaman Baru</h2>
            ...
            <div class="form-group">
                ...
                <select name="item_id" id="item_id" class="form-control" required>
                    ...
                    @foreach($items as $item)
                        <option value="{{ $item->id }}" data-available="{{ $item->available_quantity }}" {{ old('item_id') == $item->id ? 'selected' : '' }}>
        ...
        <div class="form-row">
            ...
            <div class="form-group">
                ...
                <select name="item_condition_borrow" id="item_condition_borrow" class="form-control" required>
                    <option value="baik" {{ old('item_condition_borrow') == 'baik' ? 'selected' : '' }}>Baik</option>
                    <option value="rusak ringan" {{ old('item_condition_borrow') == 'rusak ringan' ? 'selected' : '' }}>Rusak Ringan</option>
                    <option value="rusak berat" {{ old('item_condition_borrow') == 'rusak berat' ? 'selected' : '' }}>Rusak Berat</option>
                </select>
